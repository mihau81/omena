...
describe('GET /api/user/bids', () => {
  ...
  it('returns empty bids for new user', async () => {
    const { GET } = await import('@/app/api/user/bids/route');
    const { status, data } = await callRouteHandler(GET, createRequest('GET', '/api/user/bids'));

    expect(status).toBe(200);
    expect((data as Record<string, unknown[]>).bids).toEqual([]);
  });

  it('returns 401 for unauthenticated', async () => {
    _g._omenaMockSession = null;
    const { GET } = await import('@/app/api/user/bids/route');
    const { status } = await callRouteHandler(GET, createRequest('GET', '/api/user/bids'));
    expect(status).toBe(401);
  });

  it('returns 401 for admin userType', async () => {
    _g._omenaMockSession = {
      user: {
        id: user.id,
        email: user.email,
